...
def start_macro_snapshot_worker():
    """Start the Macro snapshot prewarm worker.

    Fetches all global-market data periodically in the background so that
    frontend-facing Macro APIs can serve from a pre-computed snapshot
    instead of blocking on live external API calls during user page loads.

    Controlled by:
        MACRO_SNAPSHOT_ENABLED        (default: true)  — set to 'false' to disable
        MACRO_SNAPSHOT_INTERVAL_SEC   (default: 900)   — refresh interval in seconds
    """
    import os
    if os.getenv("MACRO_SNAPSHOT_ENABLED", "true").lower() not in ("1", "true", "yes"):
        logger.info("Macro snapshot worker disabled (MACRO_SNAPSHOT_ENABLED is not true).")
        return

    # Avoid running twice with Flask reloader in debug mode.
    ...
    try:
        ...
        get_macro_snapshot_worker().start()
    except Exception as e:
        logger.error(f"Failed to start macro snapshot worker: {e}")
...
def create_app(config_name='default'):
    ...
    with app.app_context():
        ...
        # Macro snapshot prewarm — must start before any user request hits the
        # global-market endpoints so that the cache is warm on first load.
        start_macro_snapshot_worker()
