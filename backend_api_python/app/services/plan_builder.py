...
class PlanBuilderService:
    ...
    def generate_plan_draft(self, user_id: str, plan_data: dict) -> dict:
        ...
        try:
            ...
            locale = plan_data.get("locale", "zh-CN")

            if locale == 'en-US':
                lang_instruction = "Respond entirely in English. Output ONLY valid JSON."
                disclaimer_text = "This plan is a disciplined draft generated by AI, does not constitute investment advice, does not predict price trends, and does not promise returns."
            else:
                lang_instruction = "Respond entirely in Simplified Chinese. Output ONLY valid JSON."
                disclaimer_text = "本计划为 AI 生成的纪律化草案，不构成投资建议，不预测价格走势，也不承诺收益。"

            system_prompt = f"""You are QuantDinger's AI Plan Builder. Your sole purpose is to generate disciplined, structured investment plan drafts (DCA, staged entries, risk control) based on user inputs.
...
If user input is incomplete or nonsensical, return a highly conservative, generalized educational plan adhering to the JSON structure. {lang_instruction}"""
...
Generate the strict JSON output. Respond entirely in {"English" if locale == 'en-US' else "Simplified Chinese"}."""
