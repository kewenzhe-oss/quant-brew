...
def run_backtest():
    ...
    try:
        ...
        err_str = str(e)
        logger.warning(f"Backtest ValueError: {err_str}")
        # Data availability errors have their own clear message; don't prefix with "invalid parameter"
        if 'candle data' in err_str.lower() or 'no data' in err_str.lower():
            display_msg = (
                f"未能获取到该日期范围内的 K 线数据。"
                f"请确认标的 ({symbol}) 在所选时间段内有交易记录，"
                f"并尝试缩短日期范围或切换时间周期。"
                f"(原始错误: {err_str})"
            )
        else:
            display_msg = f"数值参数无效，请检查表单输入 (Invalid parameter): {err_str}"
        return jsonify({
            'code': 0,
            'msg': display_msg,
            'data': None
