...
class CryptoDataSource(BaseDataSource):
    ...
    def get_kline(
        ...
        try:
            ...
            if not ohlcv:
                logger.warning(f"CCXT returned no K-lines: {symbol_pair}; trying yfinance fallback")
                klines = self._fetch_yfinance_klines(symbol, timeframe, limit, before_time)
                if klines:
                    self.log_result(symbol, klines, timeframe)
                    return klines
                logger.warning(f"yfinance fallback returned no K-lines: {symbol}")
                return []
    ...
    def _normalize_symbol_for_yfinance(self, symbol: str) -> str:
        """
        Convert crypto pair formats to Yahoo Finance crypto tickers.
        ...
        quote = 'USD'
        if '/' in normalized:
            quote = normalized.split('/', 1)[1].strip().upper() or 'USD'

        # Yahoo Finance publishes the major stablecoin-quoted pairs as USD tickers.
        if quote in {'USDT', 'USDC', 'BUSD', 'TUSD', 'USDP', 'DAI'}:
            quote = 'USD'
    ...
    def _fetch_yfinance_klines(
        ...
        """Fetch crypto K-lines from yfinance as a geo-resilient fallback."""
        try:
            import yfinance as yf
            import pandas as pd
        except Exception as e:
            logger.warning(f"yfinance not available for crypto fallback: {e}")
            return []

        yf_symbol = self._normalize_symbol_for_yfinance(symbol)
        yf_interval = self.YFINANCE_TIMEFRAME_MAP.get(timeframe, '1d')

        try:
            total_seconds = self.calculate_time_range(timeframe, limit, buffer_ratio=1.5)
            end_time = datetime.utcfromtimestamp(before_time) if before_time else datetime.utcnow()
            start_time = end_time - timedelta(seconds=total_seconds)

            ticker = yf.Ticker(yf_symbol)
            yf_start = start_time.strftime('%Y-%m-%d')
            ...
            if df is None or df.empty:
                logger.warning(f"yfinance returned no data for {yf_symbol} ({timeframe})")
                return []

            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.get_level_values(0)

            # Yahoo does not provide a native 4h interval; fetch 1h and aggregate.
            if timeframe == '4H':
            ...
            if klines:
                logger.info(
                    f"[CryptoKline:yfinance] {symbol}->{yf_symbol} {timeframe} "
                    f"returned {len(klines)} candles"
                )
            return klines
        except Exception as e:
    ...
    def _fetch_ohlcv(
        ...
        try:
            if before_time:
                ...
                end_time = datetime.utcfromtimestamp(before_time)
                start_time = end_time - timedelta(seconds=total_seconds)
                # calendar.timegm treats the naive datetime as UTC, avoiding local-TZ shift
    ...
    def _fetch_ohlcv_fallback(
        ...
        try:
            ...
            if before_time:
                # Use UTC to match CCXT timestamps
                end_time = datetime.utcfromtimestamp(before_time)
                start_time = end_time - timedelta(seconds=total_seconds)
                since = calendar.timegm(start_time.timetuple()) * 1000
            else:
                since = calendar.timegm((datetime.utcnow() - timedelta(seconds=total_seconds)).timetuple()) * 1000
